...
namespace CFM.Log
{
    ...
    internal class LogImpl: ILog
    {
        ...
        {
            return string.Format("{0:yyyy-MM-dd HH:mm:Sse.fff} [{1}] {2} - {3}", System.DateTime.Now, level, name, message);
        }

        public virtual void Debug(object message)
        {
            if (this._factory.InUnity)
                UDebug.Log(Format(message, "DEBUG"));
        }
        ...
        {
            if (this._factory.InUnity)
                UDebug.Log(Format(message, "INFO"));
        }
        ...
        {
            if (this._factory.InUnity)
                UDebug.LogWarning(Format(message, "WARN"));
        }
        ...
        {
            if (this._factory.InUnity)
                UDebug.LogError(Format(message, "ERROR"));
        }
        ...
        {
            if (this._factory.InUnity)
                UDebug.LogError(Format(message, "Fatal"));
        }
